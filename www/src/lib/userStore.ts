...
export function subscribeActiveUser( callback: (user: ActiveUser | null) => void): () => void
{
    if (typeof window === "undefined") { return () => {}; }

    const onCustom = (e: Event) => { callback((e as CustomEvent).detail as ActiveUser | null); };
    const onStorage = (e: StorageEvent) =>
    {
        if (e.key === KEY) { callback(getActiveUser()); }
    };
