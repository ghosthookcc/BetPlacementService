...
const PLACEHOLDER_USERS = ["User1", "User2", "User3"];
...
export const UserSelector = component$(() => {
  ...
  return (
    <div class="usel">
      ...
      >
        <span class="usel__avatar" data-empty={!chosen} />
      ...
      {open.value && (
        <ul class="usel__menu">
          {PLACEHOLDER_USERS.map((name, idx) => (
            <li key={name} aria-selected={state.index === idx}>
              <button
                type="button"
                class="usel__opt"
                onClick$={() => choose(idx)}
              >
                ...
                {state.index === idx && <span class="usel__check">✓</span>}
              </button>
            </li>
          ))}
